...
public sealed class PostgresWorkspaceRepository :
    ...
    IAsyncDisposable
{
    ...
    public PostgresWorkspaceRepository(string connectionString)
    {
    ...
    public async Task SaveAsync(Workspace workspace, CancellationToken cancellationToken = default)
    {
        ...
        var snapshot = EngineeringSnapshotMapper.ToSnapshot(workspace);
        ...
        command.CommandText = """
            ...
            """;
        command.Parameters.AddWithValue("workspace_id", workspace.Id.Value);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return documentJson is null
            ? null
            : DeserializeWorkspace((string)documentJson);
    }

    public async Task<IReadOnlyCollection<Workspace>> ListAsync(CancellationToken cancellationToken = default)
    {
        await EnsureSchemaAsync(cancellationToken).ConfigureAwait(false);
        ...
        {
            workspaces.Add(DeserializeWorkspace(reader.GetString(0)));
        }
    ...
    private static Workspace DeserializeWorkspace(string documentJson)
    {
        var snapshot = JsonSerializer.Deserialize<PersistedWorkspace>(documentJson, JsonOptions)
            ?? throw new InvalidOperationException("Persisted workspace document is empty.");
