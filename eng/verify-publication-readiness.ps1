...
Test-FileContains "eng/verify-release-staging-security.ps1" "case-varied-directory" "Release staging security regression must behavior-test case-varied test-only helper directory rejection."
Test-FileContains "eng/verify-release-staging-security.ps1" "case-varied-file-prefix" "Release staging security regression must behavior-test case-varied test-only helper file-prefix rejection."
Test-FileContains "eng/verify-release-staging-security.ps1" "renamed-binary-identity" "Release staging security regression must behavior-test renamed portable executables containing the test-only helper identity."
...
Test-FileContains "eng/verify-evidence-validation.tests.ps1" "staged Agent linked UAC token rejected after evidence rebinding" "Evidence regression must reject a linked UAC token after raw and public evidence rebinding."
Test-FileContains "eng/verify-evidence-validation.tests.ps1" "staged Agent obsolete elevation compatibility field rejected" "Evidence regression must reject the removed elevation compatibility field."
...
Test-FileContains "eng/verify-ci-release-artifact-inspection.ps1" "fully-rebound-staged-linked-token-downgrade" "CI artifact inspection regression must reject a fully rebound linked-token downgrade."
...
Test-FileContains "eng/verify-release-candidate-inspection.ps1" "windows-canonical-alias-path" "Release candidate inspection verification must reject trailing-dot Windows path aliases."
Test-FileContains "eng/verify-release-candidate-inspection.ps1" "windows-superscript-device-alias-path" "Release candidate inspection verification must reject superscript Windows device-name aliases."
...
Test-FileContains "eng/verify-release-candidate-inspection.ps1" "test-only-service-token-helper-leak" "Release candidate inspection verification must reject the test-only service-token helper by its forbidden file-name prefix."
Test-FileContains "eng/verify-release-candidate-inspection.ps1" "renamed-test-only-service-token-helper-directory-leak" "Release candidate inspection verification must reject renamed test-only service-token helper payloads in case-varied helper directories."
Test-FileContains "eng/verify-release-candidate-inspection.ps1" "renamed-test-only-service-token-helper-binary-leak" "Release candidate inspection verification must reject renamed portable executables containing the test-only helper identity."
Test-FileContains "eng/verify-release-candidate-inspection.ps1" "unmanifested-test-only-service-token-helper-binary-leak" "Release candidate inspection verification must reject unmanifested helper payloads outside the release inventory."
Test-FileContains "eng/verify-release-candidate-inspection.ps1" "missing-service-token-helper-source" "Release candidate inspection verification must require the test-only helper source in the source artifact."
...
Test-FileContains "eng/invoke-run-scoped-agent-service-cleanup.ps1" "packageCacheRoot" "Agent service cleanup manifest must bind the exact immutable package-cache root."
Test-FileContains "eng/invoke-run-scoped-agent-service-cleanup.ps1" "CommonApplicationData" "Agent service cleanup must place immutable package caches under administrator-controlled CommonApplicationData."
Test-FileContains "eng/invoke-run-scoped-agent-service-cleanup.ps1" 'olo-studio-two-agent-\$role-content-\$serviceSuffix' "Agent service cleanup must derive a distinct deterministic Studio package-cache anchor for each service role."
